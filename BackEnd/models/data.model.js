const Sequelize = require("sequelize");
const dbConfig = require("../config/db.config")
const sequelize = new Sequelize(dbConfig.DB, dbConfig.USER, dbConfig.PASSWORD, {
  host: dbConfig.HOST,
  dialect: dbConfig.dialect,
  operatorsAliases: false,
});

const restaurants = sequelize.define("restaurants", {
    r_id: {
        type: Sequelize.INTEGER,
        primaryKey: true,
        autoIncrement: true
    },
    r_email: {
        type: Sequelize.STRING,
        allowNull: false,
        unique:true
    },
    r_password: {
        type: Sequelize.STRING,
        allowNull: false
    },
    r_name: {
        type: Sequelize.STRING,
        allowNull: false
    },
    r_city: {
        type: Sequelize.STRING,
    },
    r_state: {
        type: Sequelize.STRING,
    },
    r_desc: {
        type: Sequelize.TEXT,
    },
    r_contact_no: {
        type: Sequelize.BIGINT(11),
    },
    r_delivery_type: {
        type: Sequelize.ENUM,
        values: ['Delivery', 'Pickup', 'Both'],
    },
    r_start: {
        type: Sequelize.DATE,
    },
    r_end: {
        type: Sequelize.DATE,
    }
})

const customers = sequelize.define("customers", {
    c_id: {
        type: Sequelize.INTEGER,
        primaryKey: true,
        autoIncrement: true
    },
    c_name: {
        type: Sequelize.STRING,
        allowNull: false
    },
    c_email: {
        type: Sequelize.STRING,
        allowNull: false,
        unique:true
    },
    c_password: {
        type: Sequelize.STRING,
        allowNull: false
    },
    c_about: {
        type: Sequelize.TEXT,
    },
    c_profile_img: {
        type: Sequelize.STRING,
    },
    c_dob: {
        type: Sequelize.DATEONLY,
    },
    c_city: {
        type: Sequelize.STRING,
    },
    c_state: {
        type: Sequelize.STRING,
    },
    c_country: {
        type: Sequelize.STRING,
    },
    c_nick_name: {
        type: Sequelize.STRING,
    },
    c_contact_no: {
        type: Sequelize.BIGINT(11),
    }
})

const dishes = sequelize.define("dishes", {
    d_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    },
    d_name: {
        type: Sequelize.STRING,
        allowNull: false
    },
    d_ingredients: {
        type: Sequelize.TEXT,
        allowNull: false
    },
    d_price: {
        type: Sequelize.FLOAT,
        allowNull: false
    },
    d_desc: {
        type: Sequelize.TEXT,
        allowNull: false
    },
    d_category: {
        type: Sequelize.ENUM,
        values: ['Appetizer', 'Salads', 'Main Course', 'Desserts', 'Beverages'],
        allowNull: false
    },
    // d_cusine: {

    // },
    d_type: {
        type: Sequelize.ENUM,
        values: ['Veg', 'Non-Veg', 'Vegan']
    }
})

restaurants.hasMany(dishes, { foreignKey: 'r_id' })

const orders = sequelize.define("orders", {
    o_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    },
    o_status: {
        type: Sequelize.ENUM,
        values: ['Placed', 'Preparing', 'Ready', 'Picked Up', 'On the Way', 'Delivered'],
        allowNull: false
    },
    o_type: {
        type: Sequelize.ENUM,
        values: ['Pickup', 'Delivery'],
        allowNull: false
    },
    o_total_price: {
        type: Sequelize.FLOAT,
        allowNull: false
    },
    o_date_time: {
        type: Sequelize.DATE,
        allowNull: false
    },
    o_address: {
        type: Sequelize.TEXT,
        allowNull: false
    }
})

const order_dishes = sequelize.define("order_dishes", {
    od_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    }
})

orders.hasMany(order_dishes, { foreignKey: 'o_id' })
dishes.hasMany(order_dishes, { foreignKey: 'd_id' })

const fvrts = sequelize.define("fvrts", {
    f_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    }
})

customers.hasMany(fvrts, { foreignKey: 'c_id' })
restaurants.hasMany(fvrts, { foreignKey: 'r_id' })

customers.hasMany(orders, { foreignKey: 'c_id' })
restaurants.hasMany(orders, { foreignKey: 'r_id' })

const customer_address = sequelize.define("customer_address", {
    ca_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    },
    ca_address: {
        type: Sequelize.TEXT,
        allowNull: false
    }
})

customers.hasMany(customer_address, { foreignKey: 'c_id' })

const carts = sequelize.define("carts", {
    cart_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    }
})

customers.hasMany(carts, { foreignKey: 'c_id' })
dishes.hasMany(carts, { foreignKey: 'd_id' })
restaurants.hasMany(carts, { foreignKey: 'r_id' })


const restaurant_imgs = sequelize.define("restaurant_imgs", {
    ri_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    },
    ri_img: {
        type: Sequelize.STRING,
        allowNull: false
    },
    ri_alt_text: {
        type: Sequelize.STRING,
        allowNull: false
    }
})

restaurants.hasMany(restaurant_imgs, { foreignKey: 'r_id' })


const dish_imgs = sequelize.define("dish_imgs", {
    di_id: {
        type: Sequelize.INTEGER,
        allowNull: false
    },
    di_img: {
        type: Sequelize.STRING,
        allowNull: false
    },
    di_alt_text: {
        type: Sequelize.STRING,
        allowNull: false
    }
})

dishes.hasMany(dish_imgs, { foreignKey: 'd_id' })


module.exports = {sequelize, restaurants, customers, customer_address, orders, order_dishes, carts, restaurant_imgs, dish_imgs, fvrts, dishes}